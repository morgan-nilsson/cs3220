        ...
        while len(frontier.queue) > 0:
            ...
            for child in map[node.state]:
                if child[1] not in reached:
                    frontier.put(Node(child[1], node, child[0], 0))
                    reached.update({child[1]: child})

    def translate_plan_to_directions(self, plan, starting, facing, map):
        facing_directions = ["north", "east", "south", "west"]
        curr_node = map[starting]
        curr_state = starting
        curr_facing = facing
        diretional_plan = []
        for direction in plan:
            ...
            # get the stored values for the next facing
            curr_facing = curr_node[[x[0] for x in curr_node].index(direction)][2]
            curr_node = map[curr_node[[x[0] for x in curr_node].index(direction)][1]]
    ...
    def decide(self, map, state, facing, has_treasure, treasure_nodes, goal):
        ...
        if len(self.plan) > 0:
            ...
            if self.goal != "treasure" and has_treasure == False:
                ...
                self.plan = self.translate_plan_to_directions(min(plans, key=len), state, facing, map)

            elif self.goal != "end" and has_treasure == True:
                self.goal = "end"
                self.plan = self.translate_plan_to_directions(self.make_plan(map, state, "end"), state, facing, map)
            else:
                self.plan = self.translate_plan_to_directions(self.make_plan(map, state, self.goal), state, facing, map)
